...
==============================================================================
This script merges the training and inference pipelines. After QLoRA fine-tuning
finishes, it switches the SAME in-memory objects to inference mode and runs the
evaluation loop, writing the per-task result CSVs directly.

  - timemoe_encoder : frozen Time-MoE-50M feature extractor   (reused as-is)
  - model           : Qwen3-0.6B + LoRA adapters (4-bit)       (trained, then for_inference)
  - projector       : trainable MLP (d_ts -> d_llm)            (trained, then .eval())

No projector.pt is written, no merged model is saved, and no model is reloaded.
The objects produced by training are passed straight into generation.
"""
import unsloth
import logging
import os
...
# Inference-time env flags (set before any heavy generation work)
...
logging.info("Starting the combined multimodal train+infer script...")

# ============================================================
# 1. Compatibility patch for Time-MoE (shared by train & infer)
# ============================================================
from transformers import DynamicCache
if not hasattr(DynamicCache, "get_usable_length"):
    def _get_usable_length(self, new_seq_length=0, layer_idx=0):
        """Compatibility shim: old get_usable_length(new_seq_len, layer_idx) -> get_seq_length(layer_idx)."""
        return self.get_seq_length(layer_idx)
    DynamicCache.get_usable_length = _get_usable_length
    logging.info("Patched DynamicCache: added get_usable_length(new_seq_len, layer_idx) shim.")

# ============================================================
# 2. Load and Prepare the Time-MoE Encoder (FROZEN)
# ============================================================
...
timemoe_full.eval()
timemoe_encoder = timemoe_full  # full model; its input_projection turns raw floats -> hidden dim

D_TS = 384  # TimeMoE-50M d_model = 384
logging.info(f"Time-MoE loaded. Hidden dim = {D_TS}")

# ============================================================
# 3. Load Qwen3-0.6B with Unsloth + QLoRA
# ============================================================
max_seq_length = 8192
dtype = None
load_in_4bit = True
...
# ============================================================
# 4. Register <ts> as a Special Token
# ============================================================
...
# ============================================================
# 8. Custom Dataset with Embedding Splicing
# ============================================================
class TimeSeriesTextDataset(Dataset):
    def __init__(self, hf_dataset, tokenizer, timemoe_encoder, projector,
    ...
    def __getitem__(self, idx):
        ...
        # Encode time series with frozen Time-MoE
        ...
        ts_embeds = ts_embeds.squeeze(0)  # [T, 384]
...
# ============================================================
# 10. Custom Trainer (projector + splicing inside compute_loss)
# ============================================================
...
# ============================================================
# 11. Build Dataset & Training Args
# ============================================================
...
# ============================================================
# 12. Initialize Trainer with Projector in the Optimizer
# ============================================================
...
# ============================================================
# 14. Switch the SAME objects to inference mode (no save/load)
# ============================================================
logging.info("Switching to inference mode (reusing in-memory model + projector)...")

# Free optimizer / training graph before generation.
del trainer
torch.cuda.empty_cache()

FastLanguageModel.for_inference(model)  # Unsloth fast inference; LoRA adapters stay active
model.eval()
projector.eval()
...
# ============================================================
# 16. Test Datasets & Execution Loop
# ============================================================
...
output_dir = "results/time_moe_6tasks"
